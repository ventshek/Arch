...
#/etc/systemd/system/display-manager.service
    ...
    # # Creates an ISO in iso_dir, using working_dir, taking config from releng.
    # command1="mkarchiso -v -w ./working_dir -o ./iso_dir ./releng"
    # # Removes the files in working_dir.
    # command2='rm -rf ./working_dir/*'
    # # Opens the iso for testing.
    # command3='qemu-system-x86_64 -display gtk -boot d -cdrom ./iso_dir/* -m 512'

    # # Asks user if they wish to create the ISO.
    # read -e -p "Creat ISO? [Y/n] " create
    # if [[ $create == "y" || $create == "Y" || $create == "" ]]; then
    # $command1 | grep '^\[mkarchiso\] INFO:'
    # else
    # exit
    # fi

    # # Asks the user if they wish to test the ISO. 
    # read -e -p "Test ISO? [Y/n] " test
    # if [[ $test == "y" || $test == "Y" || $test == "" ]]; then
    # $command3
    # fi

    # # Asks the user if they wish to delete the work directory. 
    # read -e -p "Delete work directory? [Y/n] " delete
